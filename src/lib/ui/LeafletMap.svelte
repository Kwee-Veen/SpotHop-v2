<script lang="ts">
  ...
  onMount(async () => {
    if (browser) {
      if (data.spots && Array(data.spots)) {
        console.log("Considers this to be an array of spots");
        spots = data.spots;
        spots.forEach((spot: Spot) => {
          const popup = `Spot "${spot.name}" (${spot.latitude} ${spot.longitude}), category: ${spot.category}`;
          const newMark = L.marker([spot.latitude, spot.longitude]).bindPopup(
            popup,
          );
        ...
        console.log("Considers this to be a single spot");
        spot = data.spot;
        const popup = `Spot "${spot.name}" (${spot.latitude} ${spot.longitude}), category: ${spot.category}`;
        const newMark = L.marker([spot.latitude, spot.longitude]).bindPopup(
          popup,
      ...
      baseLayers = {
        ...
        Transportation: leaflet.tileLayer(
          "https://tile.jawg.io/jawg-matrix/{z}/{x}/{y}{r}.png?access-token={accessToken}",
          {
            attribution:
              '<a href="https://jawg.io" title="Tiles Courtesy of Jawg Maps" target="_blank">&copy; <b>Jawg</b>Maps</a> &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
            accessToken:
              "kuYRHbThoHMWdZbQx3B0ZgqX850GZnVZFf63DAKiU5SnMJa3s3LrdoINWmD2iEP9",
          },
        ),
      };
      overlays = {
        "Show Temperature": leaflet.tileLayer(
          `https://{s}.tile.openweathermap.org/map/temp_new/{z}/{x}/{y}.png?appid=0c109ad6bb8a0b5d8a284ce6061f12c6`,
          {
            attribution:
              'Map data &copy; <a href="http://openweathermap.org">OpenWeatherMap</a>',
          },
        ),
        "Show Clouds": leaflet.tileLayer(
          `https://{s}.tile.openweathermap.org/map/clouds/{z}/{x}/{y}.png?appid=0c109ad6bb8a0b5d8a284ce6061f12c6`,
          {
            attribution:
              'Map data &copy; <a href="http://openweathermap.org">OpenWeatherMap</a>',
          },
        ),
        "Show Pressure": leaflet.tileLayer(
          `https://{s}.tile.openweathermap.org/map/pressure/{z}/{x}/{y}.png?appid=0c109ad6bb8a0b5d8a284ce6061f12c6`,
          {
            attribution:
              'Map data &copy; <a href="http://openweathermap.org">OpenWeatherMap</a>',
          },
        ),
        "Show Wind": leaflet.tileLayer(
          `https://{s}.tile.openweathermap.org/map/wind_new/{z}/{x}/{y}.png?appid=0c109ad6bb8a0b5d8a284ce6061f12c6`,
          {
            attribution:
              'Map data &copy; <a href="http://openweathermap.org">OpenWeatherMap</a>',
          },
        ),
        "Show Snow": leaflet.tileLayer(
          `https://{s}.tile.openweathermap.org/map/snow/{z}/{x}/{y}.png?appid=0c109ad6bb8a0b5d8a284ce6061f12c6`,
          {
            attribution:
              'Map data &copy; <a href="http://openweathermap.org">OpenWeatherMap</a>',
          },
